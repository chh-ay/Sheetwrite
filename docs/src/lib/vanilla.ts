...
// wired to `grid.actions`. The request-aware menu demonstrates stable item IDs,
// shortcuts, predicates, and a custom callback without a second UI layer.
const grid = createGrid(host, {
  ...
  config: {
    ...
    find: true,
    contextMenu: (request) => [
      { id: "copy", action: "copy", shortcut: "Ctrl+C" },
      { id: "paste", action: "paste", shortcut: "Ctrl+V" },
      { action: "separator" },
      { id: "clear", action: "clearContents", disabled: request.cell === null },
      {
        id: "highlight",
        label: "Highlight cell",
        visible: ({ cell }) => cell !== null,
        onClick(activeGrid, cell) {
          if (cell === null) return;
          activeGrid.highlightCells([
            { sheet: cell.sheet, start: cell, end: cell, color: "#f0b42955" },
          ]);
        },
      },
    ],
